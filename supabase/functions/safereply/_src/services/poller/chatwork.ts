...
import { redis, isRedisAvailable, markRedisUnavailable } from '../../db/redis.ts';
...
export async function pollChatwork(
  ...
  try {
    ...
    if (!effectiveLineUserId) {
      const configError = 'LINE user ID not configured. Set line_user_id in request body or LINE_ALLOWED_USER_IDS.';
      console.error('[Chatwork polling]', configError);
      return {
        summary: {
          users_processed: 0,
          rooms_checked: 0,
                  ...
                  if (lineUserIdForNotification) {
                    try {
                      ...
                      await sendLineNotification(
                        ...
                        draft,
                        {
                          // ChatworkではType Cも参考通知としてLINEに送る
                          notifyTypeC: true
                        }
                      );
