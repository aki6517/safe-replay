 ...
 * LINE通知サービス
 */
import { sendTextMessage, isLineClientAvailable } from './line.ts';
import { getSupabase } from '../db/client.ts';
import type { TriageType } from '../types/triage.ts';
...
export async function sendLineNotification(
  ...
  try {
    ...
    if (triageType === 'A') {
      ...
      if (draft) {
        const { createTypeAFlexMessage } = await import('./flex-messages/type-a.ts');
        const { sendFlexMessage } = await import('./line.ts');
      ...
      const { createTypeBFlexMessage } = await import('./flex-messages/type-b.ts');
      const { sendFlexMessage } = await import('./line.ts');
