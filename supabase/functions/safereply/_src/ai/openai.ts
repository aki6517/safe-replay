...
import type { AIProvider, MessageContext, AIProviderConfig } from './provider.ts';
import type { TriageResult, TriageType } from '../types/triage.ts';
import { buildTriagePrompt, buildDraftPrompt, buildSoftenPrompt } from './prompts/index.ts';
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        if (error.status === 400 || error.status === 401 || error.status === 403) {
          ...
          if (error.status === 401) {
            try {
              const { notifyApiTokenExpired } = await import('../utils/emergency-notification.ts');
