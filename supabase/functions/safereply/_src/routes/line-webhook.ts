...
import { isLineClientAvailable, replyTextMessage } from '../services/line.ts';
import { processForwardedMessage } from '../services/message-processor.ts';
import { handleLineAction, handleEditModeMessage } from '../services/action-handler.ts';
import { isUserAllowedSync } from '../utils/security.ts';
import { getEditMode } from '../services/edit-mode.ts';
import type {
  LineWebhookRequest,
  LineWebhookEvent,
  MessageEvent,
  PostbackEvent
} from '../types/line-webhook.ts';
    ...
    for (const event of events) {
      ...
      if (eventType === 'message') {
        ...
        if (event.replyToken) {
          ...
          const welcomeMessage = 'SafeReplyへようこそ！\n\nメッセージを転送すると、AIが自動で返信ドラフトを作成します。';
          const { sendTextMessage } = await import('../services/line.ts');
