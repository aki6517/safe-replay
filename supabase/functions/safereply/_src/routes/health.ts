...
import { supabase, isSupabaseAvailable } from '../db/client.ts';
import { redis, isRedisAvailable, markRedisUnavailable } from '../db/redis.ts';
import { getUptimeSeconds } from '../utils/runtime.ts';
...
healthRoutes.get('/deep', verifyServiceKey, async (c) => {
  ...
  if (isSupabaseAvailable() && supabase) {
    try {
      ...
      if (error) {
        try {
          const { notifyDatabaseError } = await import('../utils/emergency-notification.ts');
