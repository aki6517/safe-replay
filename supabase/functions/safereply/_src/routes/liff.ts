...
import { getBaseUrlFromRequest, joinBaseUrl } from '../utils/base-url.ts';
...
liffRouter.get('/', (c) => {
  ...
  const gmailRedirectUri = joinBaseUrl(baseUrl, '/api/oauth/gmail/callback');
  const slackStartUrl = joinBaseUrl(baseUrl, '/api/oauth/slack/start');
...
liffRouter.get('/callback/success', (c) => {
  ...
  const description = provider === 'Slack'
